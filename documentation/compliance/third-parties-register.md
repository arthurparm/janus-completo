...
| OmniRoute (gateway LLM auto-hospedado) | Inferência/geração via gateway próprio | Conteúdo de prompts, contexto, metadados de uso | Configurável (self-hosted por padrão; host efetivo definido por `OMNIROUTE_BASE_URL`) | Desabilitado por padrão; se apontado para host externo, revisar ToS/DPA do gateway de destino |
