...
| MR-002 | Politica de roteamento explicita entre Postgres, vetor e grafo | P0 | M | não comprovada |
...
| AG-003 | Simulacao antes de execucao de acoes destrutivas | P0 | S | não comprovada |
...
| OQ-003 | Tracing distribuido fim-a-fim com correlacao frontend/back/worker | P1 | M | não comprovada |
...
| DX-012 | Remover código duplicado e morto (ex: tool_service_improved) | P1 | S | não comprovada |
...
| ID | Melhoria | Prioridade | Esforco | Status |
|---|---|---|---|---|
| OQ-019 | Implementar timeouts assíncronos explícitos (ex: `asyncio.wait_for`) em scripts de QA (ex: `test_debate_system.py`) para evitar CI hangs | P1 | S | aberto |
| SG-050 | Mitigar risco de vazamento de PII em ferramentas de QA e 'Shadow IT' (logs customizados como em `secure-tailscale-setup.ps1` e raw `print()` em testes) | P1 | S | aberto |
| SG-051 | Mitigar risco de code injection (Bandit B602/B603) via `shell=True` em execuções de subprocessos (e.g. `launcher_tools.py`) | P0 | S | aberto |
