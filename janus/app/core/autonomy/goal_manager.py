...
class GoalManager:
            ...
            if os.path.exists(self._db_path):
                try:
                    with sqlite3.connect(self._db_path) as conn:
                        ...
                        if result != "ok":
                            logger.critical(
                                f"SQLite CORRUPTED ({result}). Initiating Emergency Restore..."
                            )
    ...
    def _restore_from_backup(self):
        ...
        if not restored:
            logger.critical(
                "ALL BACKUPS FAILED or NOT FOUND. Creating fresh DB (Data Loss likely)."
            )
            if os.path.exists(self._db_path):
                os.rename(self._db_path, f"{self._db_path}.corrupted.{int(time.time())}")

    def _persist_change(self, goal: Goal | None, event_type: str, goal_id: str):
        """
        ACID Transaction:
