...
# === Stage 1: DRAFT ===
def _build_draft_prompt(goal: Goal, metrics: Dict[str, Any], tools: List[str], max_steps: int) -> str:
    sys = (
...
# === Stage 3: REFINE ===
def _build_refine_prompt(goal: Goal, draft_plan: List[Dict[str, Any]], critique: str, tools: List[str], max_steps: int) -> str:
    plan_str = json.dumps(draft_plan, indent=2, ensure_ascii=False)
    sys = (
...
async def build_plan_for_goal(
    ...
    # Fallback seguro
    fallback = [
        {"tool": "get_current_datetime", "args": {}, "critical": True, "retry": 0, "fallback_tool": None},
        {"tool": "get_system_info", "args": {}, "critical": True, "retry": 0, "fallback_tool": None},
    ]
...
def _build_replanning_prompt(goal: Goal, failed_step: Dict[str, Any], error_msg: str, remaining_steps: List[Dict[str, Any]], tools: List[str]) -> str:
...
def _build_verification_prompt(goal: Goal, step: Dict[str, Any], result: str, error: Optional[str]) -> str:
    sys = (
