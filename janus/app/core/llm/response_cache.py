...
def get(prompt: str, role: str, priority: str) -> Optional[Dict[str, Any]]:
    if not _ENABLED:
        return None
...
def put(prompt: str, role: str, priority: str, response: str, provider: str, model: str, ttl: Optional[int] = None, input_tokens: Optional[int] = None, output_tokens: Optional[int] = None, cost_usd: Optional[float] = None) -> None:
    if not _ENABLED:
        return
    key = _make_key(hash_prompt(prompt), role, priority)
    with _lock:
