...
async def propose_node(state: DebateState):
    ...
    result = await llm.invoke_llm(
        prompt=final_prompt, role=ModelRole.CODE_GENERATOR, priority=ModelPriority.HIGH_QUALITY
    )
    ...
    return {
        ...
        "history": [f"Proponent generated code (Iter {state['iteration']})"],
    }
...
async def critique_node(state: DebateState):
    ...
    result = await llm.invoke_llm(
        prompt=prompt, role=ModelRole.PRIMARY_ASSISTANT, priority=ModelPriority.HIGH_QUALITY
    )
    response = result.get("response", "")
    ...
    try:
        ...
        json_str = (
            match.group(1)
            if match and "json" in match.group(0)
            else (match.group(0) if match else response)
        )
    ...
    return {
        "critique": critique,
        "history": [f"Critic analyzed: Approved={critique.get('approved')}"],
    }
...
workflow.add_conditional_edges("critique", decide_node, {"propose": "propose", "finish": END})
