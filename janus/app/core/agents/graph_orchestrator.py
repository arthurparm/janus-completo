...
def get_graph():
    ...
    workflow.add_conditional_edges(
        ...
        {"worker": "worker", "human_approval": "human_approval", "finish": END},
    )
    ...
    try:
        ...
        _graph_instance = workflow.compile(
            checkpointer=checkpointer, interrupt_before=["human_approval"]
        )
        ...
        logger.warning(
            f"Failed to initialize AsyncPostgresSaver: {e}. Falling back to MemorySaver."
        )
        from langgraph.checkpoint.memory import MemorySaver

        _graph_instance = workflow.compile(
            checkpointer=MemorySaver(), interrupt_before=["human_approval"]
        )
