...
class MetaAgent:
    ...
    async def monitor_node_logic(self, state: AgentState) -> dict:
        ...
        if '"status": "unhealthy"' in str(metrics):
            issues.append(
                DetectedIssue(
                    ...
                    title="Sistema Unhealthy",
                    description="Health check retornou status unhealthy",
                    evidence={},
                ).to_dict()
            )
        ...
        return {"metrics": metrics, "detected_issues": issues, "status": "monitoring"}
            ...
            if diagnosis_obj.severity == "critical":
                new_issues.append(
                    DetectedIssue(
                        ...
                        category=IssueCategory.RELIABILITY,
                        title=diagnosis_obj.root_cause,
                        description="Diagnosed Critical Issue",
                        evidence={"confidence": diagnosis_obj.confidence},
                    ).to_dict()
                )
    ...
    async def planning_node_logic(self, state: AgentState) -> dict:
        ...
        try:
            ...
            candidate_plan = [
                ...
                for item in plan_obj.recommendations
            ]
        except Exception as e:
            logger.error(f"Planning Failed: {e}")

        return {"candidate_plan": candidate_plan, "status": "planning"}
    ...
    async def reflection_node_logic(self, state: AgentState) -> dict:
        ...
        if not plan:
            return {
                "critique": {"approved": False, "reason": "Empty Plan generated"},
    ...
    async def execution_node_logic(self, state: AgentState) -> dict:
        ...
        return {"final_plan": state.get("candidate_plan"), "status": "executed"}
    ...
    def _log_report(self, report: StateReport):
        ...
        if report.issues_detected:
            ...
            for issue in report.issues_detected:
                logger.info(f"[{issue.severity.value.upper()}] {issue.title}: {issue.description}")

        if report.recommendations:
            logger.info("\n--- RECOMENDAÇÕES ---")
            for rec in report.recommendations:
                logger.info(f"[P{rec.priority}] {rec.title}: {rec.description}")
        ...
        for rec in report.recommendations:
            ...
            if rec.priority >= 4 and rec.suggested_agent:
                ...
                if target_agent_id:
                    ...
                    logger.warning(
                        f"Meta-Agente: Agente sugerido '{rec.suggested_agent}' não encontrado para aplicar correção."
                    )
    ...
    async def start_heartbeat(self, interval_minutes: int = 60):
        ...
        async def heartbeat_loop():
            logger.info(f"Meta-Agente: Heartbeat iniciado (intervalo={interval_minutes}min)")
