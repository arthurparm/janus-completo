...
class LeafWorker:
    ...
    def __init__(
        ...
        self.name = name
        self.agent = Agent(model, system_prompt=system_prompt, result_type=WorkerResult)
    ...
    def _register_tools(self, tools: List[Callable]):
        for tool in tools:
            ...
            is_unsafe = getattr(tool, "unsafe", False) or tool.__name__ in [
                "run_command",
                "write_file",
                "read_file",
            ]
    ...
    def _create_sandboxed_wrapper(self, original_tool: Callable) -> Callable:
        ...
        async def sandboxed_tool(ctx: RunContext, *args, **kwargs) -> str:
            logger.warning(
                f"Intercepting unsafe tool {original_tool.__name__} for sandbox execution"
            )
