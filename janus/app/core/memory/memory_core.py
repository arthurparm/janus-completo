...
class MemoryCore:
        ...
        for attempt in range(max_retries):
            try:
                try:
                    ...
                    logger.info(
                        f"Coleção '{self.collection_name}' (provavelmente) não encontrada. Tentando criar..."
                    )
                ...
                if is_last:
                    logger.warning(
                        "Qdrant indisponível após várias tentativas; ativando modo offline.",
                        exc_info=e,
                    )
                    self._offline = True
                    return
                    ...
                    logger.warning(
                        f"Falha de conexão com Qdrant (tentativa {attempt + 1}/{max_retries}). Retentando em {delay:.1f}s...",
                        error=str(e),
                    )
    ...
    async def amemorize(self, experience: Experience):
        ...
        try:
            ...
            origin = "unknown"
        content_bytes = (
            len(experience.content.encode("utf-8")) if isinstance(experience.content, str) else 0
        )
        now_s = time.time()
        ...
        if (q["items"] + 1) > self._quota_max_items or (
            q["bytes"] + content_bytes
        ) > self._quota_max_bytes:
            try:
        ...
        redacted_content = experience.content
        pii_detected: list[str] = []
        if bool(getattr(self.settings, "MEMORY_PII_REDACT", True)) and isinstance(
        ...
        if not self._offline:
            try:
                ...
                logger.warning(
                    "Upsert no Qdrant falhou; armazenando apenas no cache.", exc_info=True
                )
                self._offline = True
        else:
            pass
    ...
    async def arecall(
        ...
        try:
            async with self._cache_lock:
                ...
                expired_keys = [
                    k
                    for k, v in self._short_cache.items()
                    if float(v.get("expires_at", 0)) <= now_s
                ]
                ...
                scan_limit = int(
                    getattr(
                        self.settings,
                        "MEMORY_SHORT_SCAN_MAX_ITEMS",
                        max(1, self._short_max_items // 4),
                    )
                )
                    ...
                    cache_results.append(
                        ScoredExperience(
                            ...
                            timestamp=datetime.now(
                                UTC
                            ).isoformat(),  # Cache might allow approximate timestamp if not stored
            ...
            try:
                ...
                @resilient(
                    ...
                    initial_backoff=float(
                        getattr(self.settings, "LLM_RETRY_INITIAL_BACKOFF_SECONDS", 0.5) or 0.5
                    ),
                    max_backoff=float(
                        getattr(self.settings, "LLM_RETRY_MAX_BACKOFF_SECONDS", 5.0) or 5.0
                    ),
                ...
                qdrant_results = [
                    ScoredExperience(
                        ...
                        content=decrypt_text(
                            hit.payload.get("content"), hit.payload.get("metadata")
                        ),
                        type=hit.payload.get("type") or "episodic",
                        timestamp=hit.payload.get("timestamp") or datetime.now(UTC).isoformat(),
                ...
                logger.error(
                    "qdrant_search_failed_fallback_to_cache", **error_context, exc_info=True
                )
    ...
    async def arecall_by_timeframe(
        ...
        try:
            ...
            cm = (
                _tracer.start_as_current_span("memory.search_timeframe") if _OTEL else nullcontext()
            )
            async with cm as span:  # type: ignore
                if _OTEL and span is not None:
                    try:
        ...
        results = [
            ScoredExperience(
                ...
                timestamp=h.payload.get("timestamp") or datetime.now(UTC).isoformat(),
        ...
        results = [r for r in results if within(r.metadata.get("ts_ms"))]
        if min_score is not None:
            results = [r for r in results if float(r.score or 0.0) >= float(min_score)]
        try:
    ...
    async def arecall_recent_failures(
        ...
        window = (
            ...
            else int(getattr(self.settings, "MEMORY_QUOTA_WINDOW_SECONDS", 3600))
        )
        ...
        try:
            ...
            cm = (
                _tracer.start_as_current_span("memory.search_recent_failures")
                if _OTEL
                else nullcontext()
            )
            async with cm as span:  # type: ignore
            ...
            logger.warning(
                "Busca de falhas recentes no Qdrant falhou; retornando vazio.", exc_info=True
            )
            self._offline = True
            return []
        results = [
    ...
    async def arecall_recent_lessons(
        ...
        try:
            ...
            cm = (
                _tracer.start_as_current_span("memory.search_recent_lessons")
                if _OTEL
                else nullcontext()
            )
            async with cm as span:  # type: ignore
            ...
            logger.warning(
                "Busca de lições recentes no Qdrant falhou; retornando vazio.", exc_info=True
            )
            self._offline = True
            return []
        results = [
        ...
        try:
            ...
            if self._cb.is_open():
                recommendations.append(
                    "Circuit breaker is open - consider manual reset or wait for auto-recovery"
                )
            if self._offline:
...
def encrypt_text(plain_text: str) -> (str, str | None):
    f = _get_fernet()
    if f is None:
