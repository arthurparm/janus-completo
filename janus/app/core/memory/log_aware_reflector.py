...
ERROR_PATTERNS = {
    "rate_limit": re.compile(r"(429|quota|rate.?limit|exceeded|too.?many.?requests|resource.?exhausted)", re.I),
    "timeout": re.compile(r"(timeout|timed?.?out|deadline|exceeded.?time|took.?too.?long)", re.I),
    "connection": re.compile(r"(connection|refused|unreachable|dns|network|socket|event.?loop.?is.?closed)", re.I),
    "auth": re.compile(r"(auth|unauthorized|forbidden|401|403|invalid.?key|api.?key|credentials)", re.I),
    "import": re.compile(r"(import.?error|module.?not.?found|no.?module|name.?error|nameerror|attributeerror)", re.I),
    "memory": re.compile(r"(memory|oom|out.?of.?memory|heap|allocation)", re.I),
    "validation": re.compile(r"(validation|invalid|malformed|schema|pydantic|typeerror|keyerror)", re.I),
    "tool_fail": re.compile(r"(tool.?(failed|error)|execution.?failed|agent.?error)", re.I),
    "llm_fail": re.compile(r"(llm|model|inference|generation|openai|gemini|ollama).?(failed|error)?", re.I),
...
def _parse_log_line(line: str) -> Optional[Dict[str, Any]]:
    ...
    CATCH_ALL_KEYWORDS = [
        ...
        "CRITICAL", "FATAL", "PANIC", "CRASH",
        "BAD REQUEST", "BADREQUEST", "400", "401", "403", "404", "500", "502", "503",
        "NAMEERROR", "TYPEERROR", "VALUEERROR", "KEYERROR", "ATTRIBUTEERROR",
        "IMPORTERROR", "MODULENOTFOUNDERROR", "INDEXERROR", "ZERODIVISIONERROR",
        "RUNTIMEERROR", "ASSERTIONERROR", "OSERROR", "IOERROR", "FILENOTFOUNDERROR",
        "CONNECTIONERROR", "TIMEOUT", "REFUSED", "UNREACHABLE",
        "QUOTA", "EXCEEDED", "RATE LIMIT", "TOO MANY REQUESTS",
        "INVALID", "MALFORMED", "UNEXPECTED", "UNSUPPORTED",
        "RAISE", "RAISED", "THROWN",
    ]
...
class LogAwareReflector:
    ...
    def _analyze_log_file(self, cutoff: datetime) -> List[LogError]:
        ...
        if not self._log_path or not os.path.exists(self._log_path):
            logger.warning(f"[LogReflector] Log file not found")
            return errors
