...
class CircuitBreakerMetrics:
    ...
    def record_call(self, success: bool, response_time: float = 0.0):
        ...
        if response_time > 0:
            self.average_response_time = (self.average_response_time * (self.total_calls - 1) + response_time) / self.total_calls
...
class EnhancedCircuitBreaker:
    ...
    def __init__(self, 
        ...
        self.half_open_success_threshold = half_open_success_threshold or config.half_open_success_threshold
        self.name = name
    ...
    async def call_async(self, coro_func: Callable, *args, **kwargs):
        ...
        if self.is_open():
            ...
            raise CircuitOpenError(
                f"Circuit Breaker '{self.name}' está ABERTO. Chamada bloqueada."
            )
