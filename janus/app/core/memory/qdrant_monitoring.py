...
class QdrantMonitoringService:
    ...
    def _should_attempt_recovery(self) -> bool:
        ...
        if self._last_recovery_attempt and datetime.now() - self._last_recovery_attempt < timedelta(
            minutes=5
        ):
            return False
...
_qdrant_monitoring_service: QdrantMonitoringService | None = None


async def initialize_qdrant_monitoring(
    qdrant_client: EnhancedQdrantClient,
    check_interval: int = 30,
    recovery_strategy: RecoveryStrategy = RecoveryStrategy.GRADUAL,
) -> QdrantMonitoringService:
...
def get_qdrant_monitoring_service() -> QdrantMonitoringService | None:
