...
class CircuitBreakerAlertManager:
    ...
    def create_alert(self, 
                   ...
                   failure_threshold: int,
                   recovery_timeout: int,
                   context: Optional[Dict[str, Any]] = None) -> CircuitBreakerAlert:
    ...
    def check_extended_open_duration(self, circuit_breaker_name: str, open_since: float) -> Optional[CircuitBreakerAlert]:
    ...
    def get_active_alerts(self, severity: Optional[AlertSeverity] = None) -> List[CircuitBreakerAlert]:
...
class CircuitBreakerAnalytics:
    ...
    def __init__(self, max_history_minutes: int = 60):
        ...
        logger.info("circuit_breaker_analytics_initialized", max_history_minutes=max_history_minutes)
    ...
    def record_metrics(self, circuit_breaker_name: str, metrics: CircuitBreakerMetricsSnapshot):
        ...
        while (self.metrics_history[circuit_breaker_name] and 
               self.metrics_history[circuit_breaker_name][0].timestamp < cutoff_time):
            self.metrics_history[circuit_breaker_name].popleft()
    
    def record_state_change(self, circuit_breaker_name: str, old_state: CircuitBreakerState, new_state: CircuitBreakerState):
        """Record state change."""
        state_record = {
            "timestamp": datetime.now(),
            "old_state": old_state.value,
        ...
        # Clean old data
        cutoff_time = datetime.now() - timedelta(minutes=self.max_history_minutes)
        while (self.state_history[circuit_breaker_name] and 
               self.state_history[circuit_breaker_name][0]["timestamp"] < cutoff_time):
    ...
    def analyze_failure_patterns(self, circuit_breaker_name: str) -> Dict[str, Any]:
        ...
        # Calculate trends
        recent_metrics = [m for m in metrics if (datetime.now() - m.timestamp).total_seconds() / 60 <= 15]
        older_metrics = [m for m in metrics if (datetime.now() - m.timestamp).total_seconds() / 60 > 15]
        ...
        recent_avg_response_time = sum(m.average_response_time for m in recent_metrics) / len(recent_metrics)
        older_avg_response_time = sum(m.average_response_time for m in older_metrics) / len(older_metrics)
        
        return {
            "error_rate_trend": "increasing" if recent_avg_error_rate > older_avg_error_rate else "decreasing",
            "error_rate_change": recent_avg_error_rate - older_avg_error_rate,
            "response_time_trend": "increasing" if recent_avg_response_time > older_avg_response_time else "decreasing",
            ...
            "recommendation": self._generate_recommendation(recent_avg_error_rate, recent_avg_response_time)
        }
    ...
    def get_circuit_health_score(self, circuit_breaker_name: str) -> float:
        ...
        recent_metrics = [m for m in self.metrics_history[circuit_breaker_name] 
...
class CircuitBreakerMonitoringService:
    ...
    def record_state_change(self, 
        ...
        if new_state == CircuitBreakerState.OPEN and open_since:
            extended_alert = self.alert_manager.check_extended_open_duration(circuit_breaker_name, open_since)
            if extended_alert:
                logger.warning(
    ...
    def _generate_system_recommendations(self) -> List[str]:
        ...
        if critical_alerts:
            recommendations.append(f"Address {len(critical_alerts)} critical circuit breaker alerts")
        ...
        for circuit_breaker_name in self.analytics.metrics_history.keys():
            health_score = self.analytics.get_circuit_health_score(circuit_breaker_name)
            if health_score < 30:
                recommendations.append(f"Circuit breaker '{circuit_breaker_name}' has low health score ({health_score:.1f}) - immediate attention required")
