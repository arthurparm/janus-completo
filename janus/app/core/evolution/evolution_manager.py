...
from app.core.evolution.prompts import TOOL_SPECIFICATION_PROMPT, TOOL_GENERATION_PROMPT
...
class EvolutionManager:
    ...
    async def process_next_pending(self) -> Optional[Dict[str, Any]]:
        """
    ...
    def _update_status(self, item_id: str, status: str, backlog: list, result: Any = None, error: str = None):
        for item in backlog:
            if item["id"] == item_id:
                item["status"] = status
                if result: item["result"] = str(result) # Simplify storage
    ...
    async def _specify_tool(self, request: str) -> Optional[Dict[str, Any]]:
    ...
    async def _generate_code(self, spec: Dict[str, Any]) -> Optional[str]:
