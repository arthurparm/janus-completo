...
class ReflectorAgent:
    ...
    FAILURE_KEYWORDS = [
        "error", "failed", "não consegui", "não foi possível",
        "timeout", "exception", "falha", "problema",
    ...
    DISSATISFACTION_KEYWORDS = [
        "errado", "incorreto", "wrong", "não era isso",
        "tente novamente", "try again", "não funcionou",
        "isso não ajudou", "didn't help"
    ]
    ...
    async def _fetch_recent_experiences(
        self, 
        hours_back: int, 
        limit: int
    ) -> List[Dict[str, Any]]:
            ...
            for query in search_queries:
                try:
                    results = await self.memory.arecall(
                        query=query,
                        top_k=limit // len(search_queries)
                    )
                    if results:
    ...
    def _suggest_improvement_for_pattern(self, pattern_type: str) -> Optional[str]:
