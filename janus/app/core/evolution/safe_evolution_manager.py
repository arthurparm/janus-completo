...
from app.core.evolution.janus_lab import JanusLabManager, LabConfig, LabResult
# Use LogAwareReflector that reads ACTUAL logs, not just Qdrant memory!
from app.core.memory.log_aware_reflector import LogAwareReflector, EnhancedReflectionReport
...
class SafeEvolutionManager:
    ...
    async def _attempt_safe_evolution(
        self,
        description: str,
        attempt_num: int
    ) -> EvolutionAttempt:
        ...
        try:
            ...
            lab_config = self.lab.spawn_lab(
                purpose=f"validate_evolution_{attempt_num}",
                timeout_seconds=self.LAB_TEST_TIMEOUT
            )
...
'''
            ...
            if result.success and "VALIDATION_PASSED=true" in result.test_output:
                logger.info(f"[SafeEvolution] Lab validation PASSED ✅")
                ...
                try:
                    ...
                    attempt.error = f"Apply failed: {str(e)}"
                ...
                logger.warning(f"[SafeEvolution] Lab validation FAILED ❌")
...
async def dream(hours: int = 24, dry_run: bool = False) -> SafeEvolutionSession:
    ...
    # Run session
    return await manager.run_safe_evolution_session(
