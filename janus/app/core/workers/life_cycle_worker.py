...
from app.core.infrastructure import get_broker
...
class LifeCycleWorker:
    ...
    def __init__(self, goal_manager: GoalManager, memory_service: MemoryService, interval_seconds: int = 30):
    ...
    async def _pulse(self):
        ...
        try:
            failures = await self._memory_service.recall_recent_failures(limit=5, timeframe_seconds=600)
            if len(failures) >= 3:
                logger.warning(f"ALERTA DE SISTEMA: {len(failures)} falhas recentes detectadas pelo LifeCycle.")
        except Exception:
            pass
