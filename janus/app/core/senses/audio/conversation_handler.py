...
class ConversationHandler:
    ...
    def __init__(
        ...
        max_turns: int = 50,
        on_user_speech: Optional[Callable[[str], Awaitable[str]]] = None
    ):
        """
        ...
        logger.info(
            "ConversationHandler initialized",
    ...
    async def process_user_input(self, text: str) -> Optional[str]:
        ...
        try:
            ...
            if self._context.turns >= self.max_turns:
                ...
                response += " (Note: We've reached the conversation limit. Starting fresh next time.)"
            else:
                self._state = ConversationState.LISTENING
            ...
            self._state = ConversationState.LISTENING
            return f"I had trouble processing that. Could you repeat?"
    
    def _is_end_phrase(self, text: str) -> bool:
        """Check if text signals end of conversation."""
        end_phrases = [
            "goodbye", "bye", "tchau", "adeus",
            "end conversation", "stop talking",
            "that's all", "nevermind", "cancel"
        ]
...
# Singleton instance
_handler_instance: Optional[ConversationHandler] = None
