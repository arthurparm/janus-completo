...
SYSTEM_HEALTH_SCORE = Gauge(
    "system_health_score",
    "Score geral de saúde do sistema (0-100)"
)

HEALTH_CHECK_DURATION = Gauge(
    "health_check_duration_seconds",
    "Duração do último health check",
    ["component"]
)
...
class HealthMonitor:
    ...
    async def check_component(self, component: str) -> HealthCheckResult:
        ...
        try:
            ...
            result = await asyncio.wait_for(
                asyncio.to_thread(check_func) if not asyncio.iscoroutinefunction(check_func) else check_func(),
                timeout=10.0
            )
            ...
            # Atualizar métricas
            status_value = {"healthy": 1.0, "degraded": 0.5, "unhealthy": 0.0}.get(status.value, 0.0)
    ...
    def get_system_health(self) -> Dict[str, Any]:
        ...
        healthy_count = sum(
            1 for r in self.last_results.values()
        ...
        critical_components = [
            name for name, info in self.health_checks.items()
            if info["is_critical"]
        ]

        critical_unhealthy = any(
            self.last_results.get(comp, HealthCheckResult(
                component=comp,
                status=HealthStatus.UNKNOWN,
                message="",
                details={},
...
async def check_llm_manager_health() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "status": "unhealthy",
            "message": f"Erro ao verificar LLM Manager: {str(e)}",
            "details": {}
        }
...
async def check_multi_agent_system_health() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "status": "degraded",
            "message": f"Erro ao verificar sistema multi-agente: {str(e)}",
            "details": {}
        }
...
async def check_poison_pill_handler_health() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "status": "degraded",
            "message": f"Erro ao verificar poison pill handler: {str(e)}",
            "details": {}
        }
