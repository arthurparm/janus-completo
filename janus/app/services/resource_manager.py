...
def can_schedule_training(user_id: Optional[str]) -> bool:
    if not user_id:
        return True
    budget = getattr(settings, "TRAINING_GPU_BUDGET_PER_USER", {}).get(str(user_id))
    if budget is None:
        return True
    u = _user_gpu_usage.setdefault(str(user_id), {"used": 0.0, "updated_at": datetime.utcnow().isoformat()})
    return float(u.get("used", 0.0)) < float(budget)

def record_training_usage(user_id: Optional[str], cost: float) -> None:
    if not user_id:
        return
    u = _user_gpu_usage.setdefault(str(user_id), {"used": 0.0, "updated_at": datetime.utcnow().isoformat()})
...
def compute_job_priority(user_id: Optional[str], model_type: Optional[str]) -> int:
    try:
        if not user_id:
            return 3
        budget = getattr(settings, "TRAINING_GPU_BUDGET_PER_USER", {}).get(str(user_id))
