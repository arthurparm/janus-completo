...
FEEDBACK_TOTAL = Counter(
    "janus_feedback_total",
    "Total de feedbacks recebidos",
    ["rating", "type"]
)

FEEDBACK_SCORE_GAUGE = Gauge(
    "janus_feedback_average_score",
    "Score médio de satisfação (últimas 100 respostas)"
)
...
class SatisfactionReport:
    ...
    nps_score: Optional[int]  # -100 a 100
...
class FeedbackService:
    ...
    async def get_satisfaction_report(
        ...
        top_issues = [
            ...
            if f.rating == FeedbackRating.NEGATIVE and f.comment
        ][:5]
    ...
    async def get_improvement_suggestions(self) -> List[Dict[str, Any]]:
        ...
        async with self._lock:
            negative_feedbacks = [
                f for f in self._feedbacks
                if f.rating == FeedbackRating.NEGATIVE
