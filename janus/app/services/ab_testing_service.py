...
class ABTestingService:
    def __init__(self, repo: Optional[ABExperimentRepository] = None):
    ...
    def compute_winner(self, experiment_id: int, metric_name: str = "accuracy") -> Dict[str, Any]:
        ...
        try:
            ...
            results = s.query(ExperimentResult).filter(ExperimentResult.experiment_id == experiment_id, ExperimentResult.metric_name == metric_name).all()
            ...
            for arm in arms:
                ...
                d.update({"arm_id": arm.id, "name": arm.name, "model_spec": arm.model_spec, "n": n, "mean": mean, "var": var})
