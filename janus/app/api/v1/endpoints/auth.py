...
async def supabase_exchange(payload: SupabaseExchangeRequest, repo: UserRepository = Depends(get_user_repo)):
    try:
        ...
        if not email and not sub:
            raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="email/sub missing")
