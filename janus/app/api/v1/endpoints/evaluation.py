...
async def create_experiment(req: ExperimentCreateRequest, repo: ABExperimentRepository = Depends(get_repo)):
...
async def add_arm(experiment_id: int, req: ArmCreateRequest, repo: ABExperimentRepository = Depends(get_repo)):
    arm = repo.add_arm(experiment_id, req.name, req.model_spec)
    return ArmResponse(id=arm.id, experiment_id=arm.experiment_id, name=arm.name, model_spec=arm.model_spec)

@router.get("/experiments", response_model=List[ExperimentResponse])
async def list_experiments(user_id: Optional[str] = None, repo: ABExperimentRepository = Depends(get_repo)):
    items = repo.list_experiments(user_id=user_id, limit=100)
    return [ExperimentResponse(id=e.id, name=e.name, user_id=e.user_id, status=e.status) for e in items]
