...
@router.post(
    "/execute", response_model=AgentResponse, summary="Envia uma instrução para um agente executar"
)
async def agent_execute(
    ...
    """
    logger.info(
        "Recebida requisição de execução de agente.",
    ...
    result = await service.execute_agent(
        question=request.question, agent_type=request.agent_type, http_request=http_request
    )
