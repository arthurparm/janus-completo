...
async def upload_document(
    ...
    try:
        if bool(auto_consolidate):
            consolidation = await knowledge.consolidate_document(
                user_id=uid, doc_id=result.get("doc_id"), limit=50
            )
    except Exception:
        consolidation = None
    try:
...
async def delete_document(doc_id: str, user_id: str | None = None, request: Request = None):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Falha ao excluir documento"
        )
    return {"status": "ok"}
