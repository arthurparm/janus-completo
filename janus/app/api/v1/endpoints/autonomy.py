...
from app.services.autonomy_service import AutonomyService, get_autonomy_service, AutonomyConfig
from app.core.autonomy.goal_manager import GoalManager, get_goal_manager, Goal, GoalStatus
...
class AutonomyStartRequest(BaseModel):
    ...
    plan: List[Dict[str, Any]] = Field(default_factory=list, description="Lista de passos: {'tool': str, 'args': dict}")
...
def _validate_plan_steps(plan: List[Dict[str, Any]], allowlist: List[str] | None = None, blocklist: List[str] | None = None) -> None:
...
async def start_autonomy(
    ...
    if request.plan:
        _validate_plan_steps(request.plan, allowlist=request.allowlist or [], blocklist=request.blocklist or [])
    ...
    if not ok:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="AutonomyLoop já está ativo.")
...
async def stop_autonomy(service: AutonomyService = Depends(get_autonomy_service)):
    ...
    if not ok:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="AutonomyLoop não está ativo.")
    return {"status": "stopped"}


@router.get("/status", response_model=AutonomyStatusResponse, summary="Obtém status do AutonomyLoop")
...
async def list_goals(status: Optional[str] = None, manager: GoalManager = Depends(get_goal_manager)):
...
@router.patch("/goals/{goal_id}/status", response_model=GoalResponse, summary="Atualiza status da meta")
async def update_goal_status(goal_id: str, req: GoalStatusUpdateRequest, manager: GoalManager = Depends(get_goal_manager)):
    if req.status not in {GoalStatus.PENDING, GoalStatus.IN_PROGRESS, GoalStatus.COMPLETED, GoalStatus.FAILED}:
