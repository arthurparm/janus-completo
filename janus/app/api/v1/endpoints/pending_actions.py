...
async def list_pending(
    ...
    repo: PendingActionRepository = Depends(get_repo),
):
    ...
    return [
        PendingActionDTO(
            ...
            decided_at=str(i.decided_at) if i.decided_at else None,
        )
        for i in items
    ]
...
async def approve(
    action_id: int, request: Request, repo: PendingActionRepository = Depends(get_repo)
):
    item = repo.get(action_id)
    if not item:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Ação pendente não encontrada"
        )
    try:
        ...
        if not tool or not meta:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Ferramenta inválida"
            )
        ...
        if not decision.allowed:
            raise HTTPException(
                ...
                if (decision.reason or "").lower().startswith("rate limit")
        ...
        if scopes:
            ...
            for sc in scopes:
                if not cr.has_consent(int(item.user_id), sc):
                    raise HTTPException(
                        status_code=status.HTTP_403_FORBIDDEN, detail=f"Consent required: {sc}"
                    )
    ...
    try:
        if item and item.run_id:
            ...
            ar.add_step(
                ...
                result_preview=str(result)[:500] if "result" in locals() else None,
                result_length=len(str(result)) if "result" in locals() else 0,
                success=True,
                error=None,
    ...
    return PendingActionDTO(
        ...
        decided_at=str(item.decided_at) if item.decided_at else None,
    )
