            ...
            for row in rows:
                ...
                if is_approval:
                    pending_actions.append(
                        PendingActionDTO(
                            thread_id=t_id, status="pending", message="Waiting for approval"
                        )
                    )
...
@router.post(
    "/{thread_id}/approve", response_model=PendingActionDTO, status_code=status.HTTP_202_ACCEPTED
)
...
@router.post(
    "/{thread_id}/reject", response_model=PendingActionDTO, status_code=status.HTTP_202_ACCEPTED
)
