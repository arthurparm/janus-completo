...
import {
  ...
  DeploymentPrecheckResponse,
  GPUBudgetResponse,
  GPUUsageResponse,
  ABExperimentSetResponse,
} from '../../models';
...
export class LlmApiService {
  ...
  getLLMCacheStatus(): Observable<LLMCacheStatusResponse> {
    return this.adminActions.execute<LLMCacheStatusResponse>(
      'get_cache_status_api_v1_llm_cache_status_get',
    );
  }

  getLLMCircuitBreakers(): Observable<CircuitBreakerStatus[]> {
    return this.adminActions.execute<CircuitBreakerStatus[]>(
      'get_circuit_breaker_status_api_v1_llm_circuit_breakers_get',
    );
  }
  ...
  stageDeployment(model_id: string, rollout_percent: number): Observable<DeploymentStageResponse> {
    return this.adminActions.execute<DeploymentStageResponse>(
      'stage_api_v1_deployment_stage_post',
      { payload: { model_id, rollout_percent } },
    );
  }

  publishDeployment(model_id: string): Observable<DeploymentPublishResponse> {
    return this.adminActions.execute<DeploymentPublishResponse>(
      'publish_api_v1_deployment_publish_post',
  ...
  rollbackDeployment(model_id: string): Observable<DeploymentPublishResponse> {
    return this.adminActions.execute<DeploymentPublishResponse>(
      'rollback_api_v1_deployment_rollback_post',
      { queryParams: { model_id } },
    );
  }

  precheckDeployment(model_id: string): Observable<DeploymentPrecheckResponse> {
    return this.adminActions.execute<DeploymentPrecheckResponse>(
      'precheck_api_v1_deployment_precheck_post',
      { queryParams: { model_id } },
    );
  }

  getGPUUsage(_userId: string): Observable<GPUUsageResponse> {
    return this.http.get<GPUUsageResponse>(
      this.apiContext.buildUrl(`/api/v1/resources/gpu/usage/self`),
    );
  }

  setGPUBudget(_userId: string, budget: number): Observable<GPUBudgetResponse> {
    return this.http.post<GPUBudgetResponse>(
      this.apiContext.buildUrl(`/api/v1/resources/gpu/budget`),
      { budget },
    );
  }

  setLLMABExperiment(experiment_id: number): Observable<ABExperimentSetResponse> {
    return this.adminActions.execute<ABExperimentSetResponse>(
      'set_ab_experiment_api_v1_llm_ab_set_experiment_post',
