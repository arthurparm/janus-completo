...
export interface LLMCacheStatusResponse {
  total_cached: number;
  cache_entries: LLMCacheEntry[];
}
...
export interface DeploymentPrecheckResponse {
  precheck_passed: boolean;
  bias_score: number | null;
  safety_warnings: string[] | null;
  evidence: {
    artifact_present: boolean;
    performance_metrics_present: boolean;
    fairness_metrics_present: boolean;
  };
}
export interface GPUBudgetResponse {
  user_id: string;
  budget: number;
}
...
export interface ABExperimentSetResponse {
  status: string;
  LLM_AB_EXPERIMENT_ID: number;
}
