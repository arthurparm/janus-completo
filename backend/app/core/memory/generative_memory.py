...
class GenerativeMemoryService:
    ...
    async def add_memory(
        self, content: str, type: str = "episodic", metadata: dict[str, Any] = None
    ) -> Experience:
        """
        ...
        experience = Experience(content=content, type=type, metadata=metadata)
        ...
        logger.info(
            "Memory added to generative stream", id=experience.id, importance=metadata["importance"]
        )
        return experience
    ...
    async def retrieve_memories(
        ...
        *,
        ...
        for mem in candidates:
            ...
            # Final Score
            final_score = (
                (self.alpha * recency) + (self.beta * importance) + (self.gamma * relevance)
            )
    ...
    async def _mirror_to_user_collection(self, experience: Experience) -> None:
        ...
        meta = (
            ...
            else experience.metadata.model_dump()
        )
    ...
    async def _calculate_importance(self, content: str, use_local: bool = False) -> float:
        ...
        """
        try:
            # Load prompt from prompts directory (supports subdirectory organization)
            from pathlib import Path

            prompts_dir = Path(__file__).resolve().parent.parent.parent / "prompts"
            prompt_file = prompts_dir / "memory_rating.txt"
            if not prompt_file.exists():
                matches = list(prompts_dir.rglob("memory_rating.txt"))
                if matches:
            ...
            response = await self.llm_service.invoke_llm(
                prompt=prompt, role=ModelRole.REASONER, priority=priority, timeout_seconds=10
            )
    ...
    async def prune_memories(self, retention_days: int = 60, min_importance: float = 3.0):
        ...
        try:
            result = await db.query(
                cypher, {"cutoff_date": cutoff_date, "min_importance": min_importance}
            )
            if result:
