...
NOTIFICATIONS_SENT = Counter(
    "desktop_notifications_total", "Total desktop notifications sent", ["urgency", "status"]
)
    ...
    async def notify(
        ...
        try:
            ...
            if success:
                ...
                logger.info("Notification sent", title=title, urgency=urgency.value)
            else:
...
"@
    ...
    async def notify_error(self, error_message: str) -> bool:
        ...
        return await self.notify(
            title="⚠️ Janus Alert", message=error_message, urgency=NotificationUrgency.HIGH
        )

    async def notify_reminder(self, reminder: str) -> bool:
        """Send a reminder notification."""
        return await self.notify(
            title="🔔 Reminder", message=reminder, urgency=NotificationUrgency.NORMAL
        )

    async def notify_system_status(self, status: str) -> bool:
        """Notify about system status changes."""
        return await self.notify(
            title="🤖 Janus Status", message=status, urgency=NotificationUrgency.LOW, sound=False
        )
