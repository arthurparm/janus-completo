...
class FeedbackRequest(BaseModel):
    ...
    message_id: str | None = Field(None, description="ID da mensagem (opcional)")
    rating: FeedbackRatingEnum = Field(..., description="Rating: positive, negative, ou neutral")
    feedback_type: FeedbackTypeEnum = Field(
        default=FeedbackTypeEnum.message, description="Tipo do feedback"
    )
    comment: str | None = Field(None, description="Comentário adicional", max_length=1000)
...
class QuickFeedbackRequest(BaseModel):
    ...
    comment: str | None = Field(None, description="Comentário (opcional)", max_length=500)
...
async def get_feedback_stats():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao obter estatísticas: {e!s}")
...
async def get_satisfaction_report(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao gerar relatório: {e!s}")
...
async def get_improvement_suggestions():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Erro ao obter sugestões: {e!s}")
