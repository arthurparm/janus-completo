...
async def test_citation_deadline_cancels_collection_coroutine(monkeypatch):
    ...
    result = await chat_message._collect_chat_citations_with_deadline(
        message="arquivo",
    ...
    assert result["retrieval_failure_reason"] == "retrieval_timeout"
    assert result["failure_classification"] == "citation_timeout"
