...
class TestChatService:
    ...
    async def test_start_conversation(self):
        ...
        try:
            ...
            mock_repo = MagicMock()
            mock_repo.start_conversation = MagicMock(return_value="conv-123")
            mock_repo.count_conversations = MagicMock(return_value=1)
            mock_llm = MagicMock()

            service = ChatService(repo=mock_repo, llm_service=mock_llm)

            conv_id = await service.start_conversation_async(
                persona="assistant",
                user_id="user-1",
                project_id="proj-1"
            )
            ...
            mock_repo.start_conversation.assert_called_once()
    ...
    async def test_get_history(self):
        ...
        try:
            ...
            mock_repo = MagicMock()
            mock_repo.get_conversation = MagicMock(
                return_value={
                    "persona": "assistant",
                    "messages": [
                        {"timestamp": 1.0, "role": "user", "text": "Hello"},
                        {"timestamp": 2.0, "role": "assistant", "text": "Hi there!"},
                    ],
                }
            )
            mock_llm = MagicMock()
            ...
            history = service.get_history("conv-123")
