...
**Orchestrator lifecycle telemetry**: Startup and `POST /api/v1/workers/start-all` build the same in-process worker registry. Each record contains the real asyncio handle, canonical worker name and `registered_at`, captured when the handles are registered. `/api/v1/workers/status` and `/api/v1/system/overview` derive state from those handles and expose that timestamp. They do not label task liveness as a heartbeat or invent processed-task counters; progress metrics require explicit instrumentation by the individual worker.
