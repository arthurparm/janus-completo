...
> *Completion history and detailed system surveys.*
...
* [ ] **Infrastructure — Processes & Resilience** (new)
  ...
  * [ ] **Neo4j reconnect**: implement reconnect/health gating when driver enters offline mode, avoiding getting stuck until restart.
  ...
  * [ ] **Dangerous Tools**: reinforce policy for `execute_shell`/`write_file .py` (require explicit allowlist per environment and log/audit) to prevent inadvertent use in production.
...
1) **Startup flow map (textual pipeline and criticality)**
   - FastAPI Lifespan initializes Kernel and maps services in `app.state` with legacy route compatibility.
...
3) **Coupling analysis (Manual DI)**
   - Kernel concentrates creation of repositories and services, increasing coupling and hindering isolated tests.
...
Note: listed paths already include `/api/v1` prefix.

(See source code or Swagger UI for the full list of endpoints. The static table is omitted here to avoid desynchronization.)
