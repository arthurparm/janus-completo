...
This document maintains detailed system listings, API inventories, and task breakdowns for the Janus project.

## 🧱 Analysis Tasks (Surveys)

* [x] **Batch 1 — Boot & Kernel** (Closed)
* [x] **Batch 2 — API & Endpoints** (Closed)
...
1) **Startup Flow Map (Textual pipeline and criticality)**
   ...
   - Critical steps (failures stop boot): Infrastructure and MAS Agents; "Best-effort" steps: Workers, Warm-up, Voice.

2) **Infrastructure Inventory and Operational Impact**
   - Parallel infrastructure initialization: GraphDB (Neo4j), MemoryDB (Qdrant), Broker (RabbitMQ), Redis.
   - Firebase is optional and does not block boot (non-critical failure).

3) **Coupling Analysis (Manual DI)**
   - The Kernel concentrates repository and service creation, increasing coupling and hindering isolated testing.
   - The injection flow is "eager" rather than lazy, increasing startup cost.
...
5) **Warm-up and Auto-indexing**
   ...
   - LLM background warm-up is asynchronous but still consumes resources at boot.

**Technical Recommendations (Focus on cost and performance)**
...
- [ ] Introduce a lightweight DI container (reduce coupling and improve testability).
...
1) **Complete V1 Route Inventory**
   ...
   - Real Duplicates (same method/path): `/optimization/*` and `/productivity/*` (detailed below).
...
*Note: Paths listed include the `/api/v1` prefix.*
...
2) **Contracts and Validations (Pydantic)**
   - `models/schemas.py` is used only in memory (Experience, ScoredExperience) and tasks (QueueName); the rest define local DTOs.
   - Free/Sensitive inputs without clear limits:
     ...
     - `/sandbox/execute` and `/sandbox/evaluate`: arbitrary code/expressions (requires hard gate and limits).
     ...
     - `/rag/*` and `/knowledge/*`: free query; lacks sanitization and user limits.
...
3) **HTTP Performance and Middlewares**
   ...
   - `msgpack` negotiation performs JSON decode/encode when Accept=application/msgpack; extra cost on large responses.
   - `/system/overview` and `/system/health/services` make sequential calls; could be parallelized to reduce latency.
...
4) **Governance and Versioning**
   ...
   - Naming inconsistencies: `/rag/user-chat` vs `/rag/user_chat`; `/pending_actions`; `/auto-analysis`.

5) **API Security (Keys and Headers)**
   - `X-API-Key` is optional; when absent, the entire API is exposed.
   ...
   - Recommendation: separate admin routes, require JWT/role, remove `X-User-Id` fallback, apply method allowlist.

6) **Cost Checklist for LLM Endpoints**
   - Core LLM has budgets, but `user_id`/`project_id` comes from payload; without identity authentication, limits can be bypassed.
   ...
   - `/llm/budget/summary` and `/llm/pricing/providers` exposed; ideally restricted to admin.

#### Tasks

* [ ] Remove route duplicates (`/optimization/*` and `/productivity/*`)
* [ ] Decide destination for routes not exposed in v1 router (`admin_graph/meta/resources`)
...
* [ ] Validate URL/headers in `/tools/create/from-api` and enforce limits in `/tools/create/from-function`
* [ ] Hard gate critical endpoints with JWT/role (admin) and remove `X-User-Id` fallback
* [ ] Make `X-API-Key` mandatory when no JWT is present (remove default exposed mode)
...
**Objective**: Map the service layer and explicit critical pending items in the core Janus flow.
