...
This document lists the system tasks, batches (Lotes), and detailed inventory of components, services, and API endpoints.
...
1) **Startup flow map (textual pipeline and criticality)**
   - FastAPI Lifespan initializes Kernel and maps services in `app.state` with compatibility for old routes.
...
5) **Warm-up and auto-indexing**
   - `AUTO_INDEX_ON_STARTUP=True` may cause high cost on large bases.
...
1) **Complete v1 route inventory**
   ...
   - Exposed PUBLIC_API_MINIMAL mode: /chat, /users, /profiles, /autonomy, /assistant, /autonomy/history, /consents, /pending_actions, /evaluation, /deployment, /auth, /auto-analysis, /feedback.
...
3) **HTTP Performance and Middlewares**
   ...
   - `/system/overview` and `/system/health/services` make sequential calls; could parallelize to reduce latency.
