# 📦 Janus Inventory & Detailed Surveys
>
> *Detailed analysis of infrastructure, endpoints, and components.*

This document contains the detailed breakdown of the technical surveys performed on the Janus system.

## 📌 Index

- [Lot 1 — Boot & Kernel (Detailed)](#-lot-1--boot--kernel-closed)
- [Lot 2 — API & Endpoints (Detailed)](#-lot-2--api--endpoints-closed)

---

## ✅ Lot 1 — Boot & Kernel (CLOSED)
...
1) **Startup Flow Map (textual pipeline and criticality)**
   ...
   - Critical stages (failure interrupts): infrastructure and MAS agents; "best-effort" stages: workers, warm-up, voice.
...
3) **Coupling Analysis (Manual DI)**
   ...
   - Injection flow is "eager" and not lazy, raising startup costs.
...
6) **Cost Checklist for LLM Endpoints**
   - Core LLM has budgets, but user_id/project_id come from payload; without authenticating identity, limits can be bypassed.
   ...
   - /llm/budget/summary and /llm/pricing/providers exposed; ideally restrict to admin.
...
* [ ] Make X-API-Key mandatory when no JWT (remove default exposed mode)
...
### Lot 3 - Services (LLM, Chat, RAG, Observability, Autonomy) (IN PROGRESS)
...
### Lot 4 - Repositories and Persistence (IN PROGRESS)
...
### Lot 5 - Agents, Tools, and Sandbox (IN PROGRESS)
