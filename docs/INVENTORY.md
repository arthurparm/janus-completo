...
> This document lists detailed inventories of infrastructure, API routes, and task batches for the Janus project.
...
1) **Complete v1 route inventory**
   ...
   - Routes defined, but not exposed in router v1: admin_graph, meta, resources.
...
* [ ] Decide destination of routes not exposed in router v1 (admin_graph/meta/resources)
...
* [ ] Propagate budget enforcement (USD) to the HTTP gateway per user/tenant

### Batch 3 - Services (LLM, Chat, RAG, Observability, Autonomy) (IN PROGRESS)

**Objective**: map the service layer and explicit critical pendencies in the Janus heart flow.

#### Tasks

* [ ] Impose size limits in LLMService (prompt + output)
* [ ] Impose size limits in ChatService (message + attachments)
* [ ] Standardize cost registration/estimation in the Chat → LLM path
