...
### 1. Header `X-User-Id` Exploit (Impersonation) (Feito)
...
- **Ação Recomendada:** Configurar o flag como falso nos defaults de produção e validar robustamente a criptografia do token de acesso injetado em Headers customizados. (Atualizado: A flag `AUTH_TRUST_X_USER_ID_HEADER` foi definida como `False` por padrão).

### 2. Bypass de Autenticação - Shared Workspaces (Feito)
...
- **Ação Recomendada:** Implementar checagem de Token/Access Control nas rotas do workspace, proibindo acessos anônimos para gerenciar estados do container ou memória de agentes. (Atualizado: `get_actor_user_id` foi implementado nas rotas para validar permissões).

### 3. Ausência de Rate-Limiter (DDoS/Brute-Force) (Em Andamento/Bloqueado)
...
- **Ação Recomendada:** Adicionar a notação e política de `limit` do Redis a nível de rota de login e senhas. O repositório possui uma limitação global de IP implementada, mas carece da dependência `slowapi` necessária para a implementação local na rota (adicionado à seção de Melhorias).
...
### 6. Vulnerabilidade de Execução Arbitrária de Comandos (OS Command Injection) (Feito)
...
- **Ação Recomendada:** Remover `shell=True` e usar uma lista de argumentos para chamar o processo, ex: `subprocess.Popen(["start", '""', app_name])` ou alternativas mais seguras na stdlib de execução. (Atualizado: `shell=True` removido e argumentos refatorados para formato de array com `cmd.exe`).
