...
* [ ] **RAPTOR** - *Sarthi et al., 2024*
  * **Concept**: Recursive indexing in a tree (summaries of summaries).
  * **In Janus**: Hierarchical structure of knowledge in Neo4j.
...
* [ ] **Constitutional AI** - *Bai et al., 2022 (Anthropic)*
  * **Concept**: Behavioral control through a "Constitution" (natural rules) instead of extensive manual RLHF.
...
* [ ] **FrugalGPT (LLM Cascades)** - *Chen et al., 2023*
  ...
  * **In Janus**: `ModelRouter` in infrastructure that attempts to solve with Llama-3-Local/Mini before calling DeepSeek/GPT-4.
* [ ] **DSPy (Programming with Prompts)** - *Khattab et al., 2023*
  ...
  * **In Janus**: Self-tuning pipeline of Workers' prompts based on error/success feedback.

### 🎨 HCI & Experience (The Interface)

* [ ] **Generative UI** - *Vercel AI SDK v5 / Dynaboard*
  * **Concept**: The UI is dynamically generated by the LLM to adapt to user intent (tables, charts, forms on-the-fly).
...
* **Implementation**: `ToolSynthesizerAgent`. When Janus identifies a repetitive task without a tool, it writes a Python script, validates in Sandbox, and if it works, saves it to DB as a new permanent `Tool`.
...
1. **Skeleton-of-Thought** - *Ning et al., 2023*
    * **Concept**: Generate first the skeleton (topics) of the response, then fill the content in parallel.
...
* **Qwen 2.5 72B** (The Architect):
  ...
  * *Why*: SOTA-level coding performance (similar to Claude/GPT-4), but extremely affordable.
...
* **OpenAI Data Sharing (Complimentary Tokens)**:
  * **What is it**: OpenAI offers free tokens (e.g., 250k/day) if you allow them to train with your data.
  * **The "Nightmare" Risk**: All Janus code, prompt, and strategy becomes OpenAI training property. If Janus creates something innovative, OpenAI learns.
  * **Verdict**: **Enable ONLY if the project has no commercial secrets/critical IP.** Otherwise, the cost of violated privacy >>> $10 savings.
...
* **DeepSeek Specifics**: DeepSeek V3 supports *Strict Mode* (Function Calling) via `beta` endpoint or advanced prompt engineering. We will use the DeepSeek OpenAI-compatible standard.
...
*Mandatory items for the version 1.0 launch.*
...
* [ ] **Autonomy 500 Error**: Internal Server Error (500) makes it impossible to create Strategic Goals.
...
* [ ] **State Desync**: Lack of reactivity between Backend (Redis) and Frontend (NgRx/Signals).
  ...
  - [ ] Define unique state strategy (SignalStore/Elf/NgRx) and migration.
...
* [ ] **Design System Conflict**: Remove Angular Material gradually and unify in Shadcn/UI (Tailwind) to reduce bundle size and visual inconsistencies.
