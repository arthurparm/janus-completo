...
* [ ] **Tree of Thoughts (ToT)** - *Yao et al., 2023*
  ...
  * **In Janus**: Basis for `Meta-Agent` decision process.
...
* [ ] **DSPy (Programming with Prompts)** - *Khattab et al., 2023*
  * **Concept**: Abstract prompts as optimizing parameters. The system "compiles" and improves its own prompts based on metrics.
...
* [ ] **Generative UI** - *Vercel AI SDK v5 / Dynaboard*
  * **Concept**: UI is dynamically generated by LLM to adapt to user intent (on-the-fly tables, charts, forms).
...
* **Implementation**: **Dynamic Handoffs**. Agents can transfer execution directly to other experts (`transfer_to_agent`) without passing through the Supervisor, reducing latency and bottlenecks.
...
* [ ] **Autonomy 500 Error**: Internal Server Error (500) prevents Strategic Goal creation.
