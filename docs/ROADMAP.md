...
>
> *Last Update: 01/14/2026 (Scientific & V1 Focus)*
...
- History of Completions (Archived)
...
* [ ] **Voyager** - *Wang et al., 2023*
  * **Concept**: Continuous learning via a skill library.
...
* [ ] **Self-RAG** - *Asai et al., 2023*
  ...
  * **In Janus**: `NativeGraphRAG` pipeline with a verification step.
* [x] **HyDE (Hypothetical Document Embeddings)** - *Gao et al., 2022*
  * **Concept**: Generate a hypothetical ideal answer to search for similar documents.
...
* [ ] **Constitutional AI** - *Bai et al., 2022 (Anthropic)*
  ...
  * **In Janus**: `ReflectorAgent` extension to validate outputs against safety rules (`security.yaml`) before delivery.
...
* [ ] **Generative UI** - *Vercel AI SDK v5 / Dynaboard*
  * **Concept**: The UI is dynamically generated by the LLM to adapt to user intent (on-the-fly tables, charts, forms).
...
* **Concept**: The LLM actively manages its context window like an OS manages RAM.
  * **Implementation**: `<memory_warning>` control token. When context fills, the agent is forced to decide what to "forget" (delete) or "archive" (save to Neo4j) before continuing.
...
* **Wallet A (DeepSeek API - $9.50)**:
  * **Usage**: Dedicated 100% to **Workhorse (DeepSeek V3)**.
  * **Advantage**: Lower latency (direct to source) and does not consume OpenRouter balance.
...
* **OpenAI Data Sharing (Complimentary Tokens)**:
  * **What is it**: OpenAI offers free tokens (e.g., 250k/day) if you allow them to train on your data.
  ...
  * **Verdict**: **Enable ONLY if the project has no commercial secrets/critical IP.** Otherwise, the cost of violated privacy >>> savings of $10.
...
* [x] **Security Headers Middleware**: Implement CSP, HSTS, X-Frame-Options, and X-Content-Type-Options.
  ...
  - [ ] Adjust CSP for production (reduce/avoid `unsafe-inline` and `unsafe-eval` where possible).
...
* [ ] **Complete UI Coverage**: Implement missing screens (80%+) following the new Design System (Tools, Workers, RAG).
  ...
  - [ ] Implement routes and empty/error/loading states.
...
* [ ] **Linter Bankruptcy**: `eslint-report.json` is giant (>400KB). Massive correction and stricter rules needed.
