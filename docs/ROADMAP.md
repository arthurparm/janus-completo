...
>
> *Last updated: 2026-01-14 (Scientific & V1 Focus)*
...
- [Backlog by Difficulty](#-backlog-by-difficulty)
...
- [Completion History (Archived)](#-completion-history-archived)
...
- Sweep and centralization of hardcoded settings/URLs to `environment.ts` in *Hardcoded Settings*.
...
* [ ] **Constitutional AI** - *Bai et al., 2022 (Anthropic)*
  * **Concept**: Behavior control via a "Constitution" (natural rules) instead of extensive manual RLHF.
...
* [ ] **Generative UI** - *Vercel AI SDK v5 / Dynaboard*
  * **Concept**: UI is dynamically generated by the LLM to adapt to user intent (tables, charts, forms on-the-fly).
...
* **Concept**: The LLM actively manages its context window like an OS manages RAM.
...
1. **Skeleton-of-Thought** - *Ning et al., 2023*
    * **Concept**: First generate the skeleton (topics) of the answer, then fill in the content in parallel.
...
* **Wallet A (DeepSeek API - $9.50)**:
  ...
  * **Advantage**: Lower latency (direct at source) and does not consume OpenRouter balance.
...
* **Wallet B (OpenRouter - $10.00)**:
  ...
  * **Secondary Use**: Speedster (Llama 3 Free) via free daily quota.
...
* **Reason**: Ensures 100% Schema adherence (zero parse errors), eliminating retry loops and manual validators.
...
* [ ] **Rate Limiting (Cost-Based)**: Limit users by **spend in dollars**, not just requests.
  ...
  - [ ] Propagate enforcement to HTTP gateway (budget block per endpoint/user).
...
* [ ] **UI Overhaul (Clean/Professional)**: Migrate from "Magicpunk" to a professional/minimalist SaaS aesthetic (Shadcn/UI + Tailwind).
  ...
  - [ ] Replace core components with equivalents in the new design system.
...
* [ ] **Broken Thought Stream**: The agent thought stream (SSE) is not receiving events from RabbitMQ (Chat Screen).
...
* [ ] **Autonomy 500 Error**: Internal Server Error (500) makes creating Strategic Goals impossible.
...
* [ ] **State Desync**: Lack of reactivity between Backend (Redis) and Frontend (NgRx/Signals).
  - [ ] Identify duplicated sources of truth (store/local state/cache).
...
* [ ] **Database Migration Pipeline**: Finalize transition to Alembic (abandon manual scripts).
  ...
  - [ ] Document upgrade/downgrade operational flow.
