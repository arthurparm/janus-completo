...
- Sweep and centralize hardcoded settings/URLs to `environment.ts` in *Hardcoded Settings*.
...
*Architecture based on 13+ seminal papers that ground Janus intelligence.*
...
* [x] **Reflexion** - *Shinn et al., 2023*
  * **Concept**: Agents verbalize errors and store lessons in short-term memory.
...
* [ ] **MetaGPT** - *Hong et al., 2023*
  ...
  * **In Janus**: Rigid role definitions (Product Manager, Architect, Engineer).
* [ ] **CAMEL** - *Li et al., 2023*
...
* [ ] **Constitutional AI** - *Bai et al., 2022 (Anthropic)*
  * **Concept**: Behavior control via "Constitution" (natural rules) instead of extensive manual RLHF.
...
* **Implementation**: `ToolSynthesizerAgent`. When Janus identifies a repetitive task without a tool, it writes a Python script, validates it in Sandbox, and if it works, saves it to DB as a new permanent `Tool`.
...
* **Concept**: LLM actively manages its context window like an OS manages RAM.
  * **Implementation**: `<memory_warning>` control token. When context fills up, the agent is forced to decide what to "forget" (delete) or "archive" (save to Neo4j) before continuing.
...
1. **Flow Engineering / AlphaCodium** - *CodiumAI, 2024*
    ...
    * **In Janus**: `CoderAgent` refactoring to follow this rigid StateFlow (increases accuracy from ~19% to ~44%).
...
* **Reason**: Ensures 100% Schema compliance (zero parse errors), eliminating retry loops and manual validators.
...
* [ ] **State Desync**: Lack of reactivity between Backend (Redis) and Frontend (NgRx/Signals).
  ...
  - [ ] Define single state strategy (SignalStore/Elf/NgRx) and migrate.
...
* [ ] **Smart Model Routing**: Router that chooses between Local/API based on task complexity (Savings).
