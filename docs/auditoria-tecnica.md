...
### 11. Quebra de Isolamento no Tooling e Ausência de Timeouts (Resiliência)
**Descrição:** Observou-se que scripts contidos na pasta `tooling/` (como `extract_api_inventory.py`, `run_full_process_validation.py`) não estabelecem timeouts consistentes para chamadas HTTP ou bypassam a suíte padrão de CI (e.g. `test_debate_system.py`, `seed-repro-scenarios.ps1`). Isso introduz um risco de hanging no pipeline e prejudica a validação de regressão unificada (DX-013).
**Evidências:**
- `qa/test_api_visibility_endpoints.py`, `tooling/run_api_e2e_all.py` contém imports de `requests` frequentemente com uso sem definição restrita e unificada de `timeout`, ou bypass de execuções controladas.
- O script `tooling/test_debate_system.py` roda de forma isolada `asyncio.run(main())` sem integração com `pytest`.

**Próximos passos:**
- Documentar regras na documentação do desenvolvedor impondo timeout nas requisições.
- Portar e adaptar `test_debate_system.py` e scripts similares para dentro da estrutura `qa/`. Adicionado DX-013 e SG-033 no `melhorias-possiveis.md`.

### 12. Componentes God Object no Frontend (Manutenibilidade)
**Descrição:** O componente central da interface do usuário de conversas concentra uma quantidade absurda de responsabilidades.
**Evidências:**
- O arquivo `frontend/src/app/features/conversations/conversations.ts` atinge em média 1700 linhas.

**Próximos passos:**
- Aplicar o padrão Container/Presenter. Adicionado DX-014 ao backlog técnico.

### 13. Riscos Críticos do Host Windows e Conexões Desprotegidas (Segurança e Infra)
**Descrição:** Múltiplas configurações expõem portas e serviços a vetores de ataque dentro da mesma rede. Além disso, a falha do RabbitMQ ocorre sem telemetria forte, caindo o serviço silenciosamente (Fail-open) num cenário onde o RabbitMQ reseta a conexão (`[Errno 111]`). Além disso, o Windows Agent escuta ativamente em 0.0.0.0.
**Evidências:**
- `backend/app/core/infrastructure/message_broker.py`: O broker reporta falha de conexão e continua em "offline_mode" sem alarmes ativos (SG-030).
- `backend/windows_agent.py`: `uvicorn.run(app, host="0.0.0.0", port=5001)` (SG-032).

**Próximos passos:**
- Alterar hardcodes de host de rede e amarrar `127.0.0.1` ou proxies autenticados. Alertar ativamente na inicialização no RabbitMQ. Adicionadas as issues SG-030 e SG-032 no `melhorias-possiveis.md`.

### 14. Análise Insegura de XML e Risco LGPD Secundário (AppSec)
**Descrição:** Há uma falha de parser no processamento de documentos DOCX que herda bibliotecas XML nativas do Python vulneráveis.
**Evidências:**
- `backend/app/services/document_parser_service.py`: Utiliza o `xml.etree.ElementTree.fromstring` para parsear DOCX contents, exposto a ataques XML (XXE). (SG-031).

**Próximos passos:**
- Substituir a extração XML do `document_parser_service.py` por `defusedxml.fromstring` ou outro parser seguro. Adicionado SG-031 no `melhorias-possiveis.md`.
