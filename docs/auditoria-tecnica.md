...
### 11. Refatoração de Frontend e Novos Componentes UI
**Descrição:** Observou-se a introdução de vários componentes de UI (`system-hud`, `toast`, `ui-badge`, `ui-card`, `ui-table`) e refatoração de estilos, adicionando complexidade ao frontend sem um correlato aumento na cobertura de testes para os novos componentes (como alertado no build logs, onde alguns componentes como `UiBadgeComponent` sequer são usados).
**Evidências:**
- `frontend/src/app/shared/components/ui/`
- Build warnings do Angular relativas à declaração de `UiBadgeComponent` e `UiButtonComponent` não utilizados.
**Próximos passos:**
- Limpar imports e declarações não utilizadas nos templates.
- Aumentar a cobertura de testes (vitest) para os novos componentes de UI (`ui-card`, `ui-table`, `toast`).
- Registrar tarefa OQ-018 no `melhorias-possiveis.md`.
### 12. Explosão de Arquivos de Prompt Mapeados para Subsistemas
**Descrição:** Foram introduzidos dezenas de arquivos `.txt` (convertidos/organizados em `.md`) em `.codex/prompts/` relativos aos subsistemas do Janus. Não há garantias de que esses prompts estejam acoplados a uma validação rigorosa de CI de prompts (Prompt Engineering Evaluation).
**Evidências:**
- Commit `ed4d2d2` (`Merge pull request #128`).
**Próximos passos:**
- Incluir no roadmap uma validação estática de template variables (`{{ var }}`) para os arquivos de prompt para evitar runtime errors na injeção.
- Registrar tarefa AI-017 no `melhorias-possiveis.md`.
## Achados do dia
