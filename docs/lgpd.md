...
### Próximos Passos
1. **Remover Trust em Headers Simples:** Desativar `AUTH_TRUST_X_USER_ID_HEADER` em produção **e** refatorar todas as rotas que leem `X-User-Id` diretamente do request (ex: `productivity.py`, `documents.py`, `memory.py`, `deployment.py`, `users.py`, `consents.py`, `rag.py`, `chat/deps.py`) para usar exclusivamente `request.state.actor_user_id` resolvido pelo middleware de autenticação. Apenas desabilitar o flag não é suficiente, pois essas rotas ignoram `get_actor_user_id` e leem o header diretamente.
