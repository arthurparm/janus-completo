...
- **Captura de Tela Indiscriminada (SG-022):** A interface OS (`backend/windows_agent.py`) não possui rotinas de "Data Minimization" ou ofuscação de PII para capturas de tela dos monitores que podem conter informações sensíveis do usuário e de terceiros.
- **Vazamento PII de Áudio via Janus Daemon (SG-023):** As interfaces de captura de voz/comando pelo Daemon gravam/loggam os payloads de áudio textuais integrais sem mascaramento prévio.
...
3. Impor validação e restrição/redação no tratamento de capturas de tela e gravação/conversão STT dos comandos de voz.
