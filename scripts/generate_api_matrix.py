...
Inputs:
- OpenAPI from running backend (preferred) or api_inventory.json fallback
- api_test_results.json (smoke/scenario execution results)
- endpoint references found in tests/*.py and janus/tests/*.py
...
INVENTORY_FALLBACK = ROOT / "api_inventory.json"
SMOKE_RESULTS = ROOT / "api_test_results.json"
OUT_JSON = ROOT / "docs" / "qa" / "api-endpoint-matrix.json"
OUT_MD = ROOT / "docs" / "qa" / "api-endpoint-matrix.md"
...
def load_endpoints() -> tuple[list[dict[str, Any]], str]:
    ...
    if not INVENTORY_FALLBACK.exists():
        raise FileNotFoundError(
            "No openapi live response and api_inventory.json not found."
        )
...
def discover_test_endpoint_refs() -> set[str]:
    refs: set[str] = set()
    files = list((ROOT / "tests").rglob("*.py")) + list((ROOT / "janus" / "tests").rglob("*.py"))
...
def render_markdown(matrix: dict[str, Any]) -> str:
    ...
    md.append("- Regenerate command: `python scripts/generate_api_matrix.py`")
    md.append("")
    md.append("## Summary")
    md.append("")
