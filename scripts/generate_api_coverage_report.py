...
Outputs:
- artifacts/qa/api_coverage_report.json
- artifacts/qa/api_coverage_report.md
- Optional Docker evidence files:
  - artifacts/qa/docker_evidence.json
  - artifacts/qa/janus_api_log_tail.txt
"""
...
DEFAULT_MATRIX_JSON = ROOT / "docs" / "qa" / "api-endpoint-matrix.json"
DEFAULT_REPORT_JSON = ROOT / "artifacts" / "qa" / "api_coverage_report.json"
DEFAULT_REPORT_MD = ROOT / "artifacts" / "qa" / "api_coverage_report.md"
DEFAULT_DOCKER_EVIDENCE_JSON = ROOT / "artifacts" / "qa" / "docker_evidence.json"
DEFAULT_DOCKER_LOG_TAIL = ROOT / "artifacts" / "qa" / "janus_api_log_tail.txt"
...
def load_matrix(path: Path) -> dict[str, Any]:
    if not path.exists():
        raise FileNotFoundError(
            f"Endpoint matrix not found: {path}. Run `python scripts/generate_api_matrix.py` first."
        )
