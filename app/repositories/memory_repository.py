...
class MemoryRepository:
    ...
    async def search_experiences(self, query: str, limit: Optional[int] = 10) -> List[Dict[str, Any]]:
    ...
    async def search_filtered(self, query: Optional[str], filters: Dict[str, Any], limit: Optional[int] = 10) -> List[Dict[str, Any]]:
        logger.debug("Busca filtrada no repositório de memória", query=query, filters=filters, limit=limit)
        try:
            return await self._db.arecall_filtered(query=query, filters=filters, limit=limit)
        except Exception as e:
            logger.error("Erro no repositório ao buscar com filtros", exc_info=e)
            raise

    async def search_by_timeframe(self, query: Optional[str], start_ts_ms: Optional[int], end_ts_ms: Optional[int], limit: Optional[int] = 10) -> List[Dict[str, Any]]:
        logger.debug("Busca por janela temporal", query=query, start_ts_ms=start_ts_ms, end_ts_ms=end_ts_ms, limit=limit)
        try:
            return await self._db.arecall_by_timeframe(query=query, start_ts_ms=start_ts_ms, end_ts_ms=end_ts_ms, limit=limit)
        except Exception as e:
            logger.error("Erro no repositório ao buscar por janela temporal", exc_info=e)
            raise

    async def search_recent_failures(self, limit: Optional[int] = 10, timeframe_seconds: Optional[int] = None) -> List[Dict[str, Any]]:
        logger.debug("Busca falhas recentes", limit=limit, timeframe_seconds=timeframe_seconds)
        try:
            return await self._db.arecall_recent_failures(limit=limit, timeframe_seconds=timeframe_seconds)
        except Exception as e:
