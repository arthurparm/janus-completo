...
from app.core.agents.meta_agent import get_meta_agent, StateReport
...
def get_status():
    ...
    try:
        ...
        is_active = (
                meta_agent._heartbeat_task is not None
                and not meta_agent._heartbeat_task.done()
        )
        last_report_dict = (
            meta_agent.last_report.to_dict() if meta_agent.last_report else None
        )
...
async def run_analysis_cycle():
    ...
    try:
        ...
        if "error" in report.summary.lower():
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail={"message": "Ciclo de análise executado com erro.", "report": report.to_dict()}
            )
        return report
    except Exception as e:
